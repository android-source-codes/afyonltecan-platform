...
#include "PicturePileLayerContent.h"//4.2 Merge
...
//    m_dirtyRegion.setEmpty();//4.2 Merge : removed in 4.2
...
void GraphicsLayerAndroid::updatePositionedLayers()
{
    ...
    if (view->isPositioned() && view->style()->position() == FixedPosition) {
		...
		// SAMSUNG CHANGE ++ : tvn24.pl, headerTop menu incorrect position.
		// Using layerLayer x,y is more correct than postion values. this is refer to ICS logic.
        // WAS:IntPoint renderLayerPos(position().x(), position().y());
...
// 4.2 Merge END >>
    ...
    //SAMSUNG CHANGES
    //if (m_contentLayer->isFixedBackground()) {   //SAMSUNG CHANGES
...
int GraphicsLayerAndroid::getContentLayerId() {
    if ( m_contentLayer )
        return m_contentLayer->uniqueId();
    else
        return 0;
}
...
bool GraphicsLayerAndroid::repaint()
{
    ...
    if (!gPaused && m_haveContents && m_needsRepaint && !m_image) {
        ...
        if (m_foregroundLayer && !m_contentLayer->isFixedBackground()) {
            ...
            if (!paintContext(m_contentLayer, m_contentLayerContent))//4.2 Merge
                return false;
            ...
            // Update the foreground layer size.
 			...
 			// SAMSUNG CHANGE ++ : m.daum.net, m.news.naver.com, display broken while flicking the news content.
            // WAS: m_foregroundLayer->setSize(contentsRect.width(), contentsRect.height());
            IntSize clipSize = layer->renderer()->absoluteBoundingBoxRect().size();
            m_foregroundLayer->setSize(max(contentsRect.width(),max(clipSize.width(),(int)m_size.width())),
                max(contentsRect.height(),max(clipSize.height(),(int)m_size.height())));
			// SAMSUNG CHANGE --
...
// 4.2 Merge END >>
            ...
            (void) paintContext(m_foregroundLayer, m_foregroundLayerContent);//4.2 Merge
...
// 4.2 Merge END >>
            ...
            if (isReplicated()) {
                ...
                if (replicatedLayer->maskLayer()) {
                     ...
                     mask->paintContext(mask->m_contentLayer, mask->m_contentLayerContent);//4.2 Merge
                }
            }

            // If there is no contents clip, we can draw everything into one
            // picture.
            bool painting = paintContext(m_contentLayer, m_contentLayerContent);//4.2 Merge
            if (!painting)
                return false;
