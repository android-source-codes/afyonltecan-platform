...
//SISO_HTMLComposer Start
   if(renderer() && renderer()->document() && renderer()->document()->settings()  && renderer()->document()->settings()->editableSupportEnabled()){
       if(sPos==ePos && 0==r.width())
           r.setWidth(1);
   }
//SISO_HTMLComposer End
...
{    if((renderer()==NULL) || (renderer()->style()==NULL))	return;
...
//SISO_HTMLComposer start
    ...
    {
        IntRect compositionRect;    
        if(underline.isHighlightColor)
        {
            Path path;
            IntRect tempCompositionRect(tx + start , ty , width , height());
            compositionRect = tempCompositionRect;
            ctx->fillTransparentRect(compositionRect , underline.highLightColor );
        }
        else
        {   
            IntRect tempCompositionRect(tx + start , ty + height() - lineThickness + 1 , width , lineThickness);    
            compositionRect = tempCompositionRect;
			...
			if(isLeftToRightDirection())
            	ctx->drawLineForText(IntPoint(tx + start, ty + height() - lineThickness), width, textRenderer()->document()->printing());
           		...
           		ctx->drawLineForText(IntPoint(tx , ty + height() - lineThickness), width, textRenderer()->document()->printing());
        }
        renderer()->document()->frame()->view()->invalidateRect(compositionRect);
