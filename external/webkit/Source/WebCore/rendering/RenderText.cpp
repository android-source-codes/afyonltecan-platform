...
{
	return (applyRTL(m_text));
}

bool RenderText::applyRTL(String text)
{
	int length = text.length();
	const UChar * srsStr = text.characters();			
	bool result = false; 
	
	UChar *tmpBuff = (UChar *)malloc((length+1)<<1);
	memset(tmpBuff, 0, (length+1)<<1);
	memcpy(tmpBuff, srsStr, length<<1);		

	BidiCheck *bC = new BidiCheck();
	if(bC->applyRTLForString((unsigned short *)tmpBuff, length))
