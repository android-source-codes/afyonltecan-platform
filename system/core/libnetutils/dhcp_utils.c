...
int static_do_request(const char *interface,
                    const char *ipaddr,
                    const char *gateway,
                    const int prefixLength)
{
    ...
    int prefx = prefixLength;

    //ALOGI("static_do_request [%s] [%s] [%d]\n", ipaddr, gateway, prefixLength);
    if (prefixLength == 0) prefx = 24;

    snprintf(daemon_prop_name, sizeof(daemon_prop_name), "%s_%s",
            DAEMON_PROP_NAME,
            interface);

    snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-S ip_address=%s/%d -S routers=%s %s", DAEMON_NAME,
        interface, ipaddr, prefx, gateway, interface);

    //ALOGI("daemon_cmd : %s\n", daemon_cmd);
    memset(prop_value, '\0', PROPERTY_VALUE_MAX);
    property_set(ctrl_prop, daemon_cmd);
    usleep(200000); //wait_for_configuration
    return 0;
}
