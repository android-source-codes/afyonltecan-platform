...
{
#ifdef WITH_VERBOSE
  fluid_settings_register_int(settings, "synth.verbose", 1, 0, 1,
...
fluid_synth_init(void)
{
  ...
  fluid_mod_set_amount(&default_reverb_mod, 600);                /* Amount: 200 ('tenths of a percent') */
  ...
  fluid_mod_set_amount(&default_chorus_mod, 400);                /* Amount: 200 ('tenths of a percent') */
...
new_fluid_synth(fluid_settings_t *settings)
{
  ...
  FLUID_LOG(FLUID_DBG, "Start time : %d.", synth->start);
           ...
           && (data[1] == synth->device_id || data[1] == MIDI_SYSEX_DEVICE_ID_ALL))
  {
      ...
      if (data[2] == 0x04 && data[3] == 0x01)
      {
          ...
          * Convert (int)[0-127] to volume range (float)[0.0-10.0] */
          float gain = ((data[5] & 0x7F) / 12.70)/5 ;
