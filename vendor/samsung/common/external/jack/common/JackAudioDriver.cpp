...
void JackAudioDriver::UpdateLatencies()
{
    ...
    for (int i = 0; i < fCaptureChannels; i++) {
        input_range.max = input_range.min = fBufferSize + fCaptureLatency;
    ...
    for (int i = 0; i < fPlaybackChannels; i++) {
        ...
        if (fEngineControl->fSyncMode) {
            output_range.max = output_range.min += fBufferSize;
        } else {
            output_range.max = output_range.min += fBufferSize * 2;
        }
        fGraphManager->GetPort(fPlaybackPortList[i])->SetLatencyRange(JackPlaybackLatency, &output_range);
        if (fWithMonitorPorts) {
            monitor_range.min = monitor_range.max = fBufferSize;
...
int JackAudioDriver::Attach()
{
    ...
    jack_log("JackAudioDriver::Attach fBufferSize = %ld fSampleRate = %ld", fBufferSize, fSampleRate);
    ...
    for (i = 0; i < fCaptureChannels; i++) {
        ...
        if (fEngine->PortRegister(fClientControl.fRefNum, name, JACK_DEFAULT_AUDIO_TYPE, CaptureDriverFlags, fBufferSize, &port_index) < 0) {
    ...
    for (i = 0; i < fPlaybackChannels; i++) {
        ...
        if (fEngine->PortRegister(fClientControl.fRefNum, name, JACK_DEFAULT_AUDIO_TYPE, PlaybackDriverFlags, fBufferSize, &port_index) < 0) {
            jack_error("driver: cannot register port for %s", name);
            return -1;
        }
        port = fGraphManager->GetPort(port_index);
        ...
        if (fWithMonitorPorts) {
            ...
            if (fEngine->PortRegister(fClientControl.fRefNum, name, JACK_DEFAULT_AUDIO_TYPE, JackPortIsOutput, fBufferSize, &port_index) < 0) {
...
int JackAudioDriver::Write()
{
    for (int i = 0; i < fPlaybackChannels; i++) {
        if (fGraphManager->GetConnectionsNum(fPlaybackPortList[i]) > 0) {
            ...
            int size = sizeof(jack_default_audio_sample_t) * fBufferSize;
            // Monitor ports
...
jack_default_audio_sample_t* JackAudioDriver::GetInputBuffer(int port_index)
{
    return fCapturePortList[port_index]
        ? (jack_default_audio_sample_t*)fGraphManager->GetBuffer(fCapturePortList[port_index], fBufferSize)
        : NULL;
}
...
jack_default_audio_sample_t* JackAudioDriver::GetOutputBuffer(int port_index)
{
    return fPlaybackPortList[port_index]
        ? (jack_default_audio_sample_t*)fGraphManager->GetBuffer(fPlaybackPortList[port_index], fBufferSize)
        : NULL;
}
...
{
    return fPlaybackPortList[port_index]
        ? (jack_default_audio_sample_t*)fGraphManager->GetBuffer(fMonitorPortList[port_index], fBufferSize)
        : NULL;
}
