...
int JackAlsaDriver::SetBufferSize(jack_nframes_t buffer_size)
{
    ...
    if (res == 0) { // update fEngineControl and fGraphManager
        ...
        // Restore old values
        alsa_driver_reset_parameters((alsa_driver_t *)fDriver, fBufferSize,
                                     ((alsa_driver_t *)fDriver)->user_nperiods,
                                     ((alsa_driver_t *)fDriver)->frame_rate);
    }
...
void JackAlsaDriver::UpdateLatencies()
{
    ...
    for (int i = 0; i < fPlaybackChannels; i++) {
        ...
        range.min = range.max = (alsa_driver->frames_per_cycle * (alsa_driver->user_nperiods - 1)) +
                         ((fEngineControl->fSyncMode) ? 0 : fBufferSize) + alsa_driver->playback_frame_latency;
...
int JackAlsaDriver::Attach()
{
    ...
    jack_log("JackAlsaDriver::Attach fBufferSize %ld fSampleRate %ld", fBufferSize, fSampleRate);
    ...
    for (int i = 0; i < fCaptureChannels; i++) {
        ...
        if (fEngine->PortRegister(fClientControl.fRefNum, name, JACK_DEFAULT_AUDIO_TYPE, (JackPortFlags)port_flags, fBufferSize, &port_index) < 0) {
    ...
    for (int i = 0; i < fPlaybackChannels; i++) {
        ...
        if (fWithMonitorPorts) {
            ...
            if (fEngine->PortRegister(fClientControl.fRefNum, name, JACK_DEFAULT_AUDIO_TYPE, MonitorDriverFlags, fBufferSize, &port_index) < 0) {
...
retry:
    ...
    if (nframes != fBufferSize)
        jack_log("JackAlsaDriver::Read warning fBufferSize = %ld nframes = %ld", fBufferSize, nframes);
...
{
    return alsa_driver_write((alsa_driver_t *)fDriver, fBufferSize);
}
...
void JackAlsaDriver::ClearOutputAux()
{
    for (int chn = 0; chn < fPlaybackChannels; chn++) {
        jack_default_audio_sample_t* buf =
            (jack_default_audio_sample_t*)fGraphManager->GetBuffer(fPlaybackPortList[chn], fBufferSize);
        memset(buf, 0, sizeof (jack_default_audio_sample_t) * fBufferSize);
    }
}
...
jack_nframes_t JackAlsaDriver::frame_time() const
{
    ...
    return timer.Time2Frames(GetMicroSeconds(), fBufferSize);
}
